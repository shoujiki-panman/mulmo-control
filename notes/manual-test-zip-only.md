...
- [ ] **「開く」が押せた** ← これが満たせたら #15 は閉じられる
- [ ] メニューバーにアイコンが出た

**訂正（2026-08-22）。** ここは当初「警告が出ずに開く」と書いていたが、それは厳しすぎる。
公証済みでも、隔離マークが付いたものを初めて開くときは

> 「Mulmo Control」はインターネットからダウンロードされました。開いてもよろしいですか？

という確認が出る。これは正常で、**「開く」ボタンがある。**8月に出た

> マルウェアが含まれていないことを検証できませんでした
> ［ゴミ箱に入れる］［完了］

とは別物で、あちらには「開く」が無い。**選択肢に「開く」があるかどうかで見分ける。**

出なかった場合、何と出たかをそのまま記録する。「開発元を確認できません」と
「マルウェアが含まれていないことを検証できませんでした」も別の状態で、原因が違う。
...
### 2026-08-22 の実測（v1.0.49）

Chrome で実際にダウンロードして測った。**8月11日と結果が反転している。**

| 見たもの | 8/11（自己署名） | 8/22（署名＋公証） |
|---|---|---|
| zip の隔離マーク | 付く | 付く（`0081;...;Chrome;...`） |
| .app の隔離マーク | 付く | 付く（`0281;...`） |
| `spctl -a -vv --type execute` | **rejected** | **accepted** / `source=Notarized Developer ID` |
| `stapler validate` | — | `The validate action worked!` |
| `syspolicy_check distribution` | — | `App passed all pre-distribution checks` |

**隔離マークが付いた状態で accepted。**残るは、ダブルクリックしたときに出る確認で
「開く」が押せることの目視だけ。
