...
なった。**番号は再利用しない。** 次に足すなら 172 から。

外した理由は「危険だったから」ではない。**求められていない機能だった**（#160）。
守る対象が消えたので、守る項目も消す。

## O. 置き場所と公開範囲（vibe coding のデプロイ）

元にした記事: https://zenn.dev/singularity/articles/vibe-coding-deploy-and-security

このアプリは**公開範囲 A・第1段**（利用者それぞれの Mac で動く。公開するものはゼロ）。
宣言は `CLAUDE.md` の2行で、`check-deploy.sh` が読む。ここで見るのは「その段に
留まっているか」と「段を上げるなら宣言と守りが先に来ているか」。段を上げる判断は人が
する。AI に「〇〇できるようにして」と頼むと最短のコードが返り、最短はたいてい外に
置く形なので、機械で止める。

`check-deploy.sh` は `check.sh` から呼ばれるほか、`.claude/settings.json` の hook が
ファイルを書くたびに単独で回す（grep しかしないので zsh だけで動く）。

| # | gate | 内容 | 状況 |
|---|---|---|---|
| 164 | fast | 「誰に見せるか」を1文で宣言してから置く（決めないと D のつもりがないのに D になる） | ✅ `CLAUDE.md` に `公開範囲:` と `段:` が**1行ずつ**あり、A〜D / 1〜4 であることを見る。0行・2行・E で落ちることを実測 |
| 165 | fast | ブラウザ用でない鍵（API キー・`service_role`・Bot トークン・Webhook URL）をコードに書かない | ✅ 名前ではなく**鍵の形**で探す（OpenAI / Anthropic / AWS / Google / GitHub / Slack / Telegram / JWT / PEM）。コメントに書いても漏れるので、ここだけはコメントを捨てない。秘密の名前に長い定数を代入する形も落とす |
| 166 | fast | ブラウザに配る名前（`VITE_` / `NEXT_PUBLIC_`）に秘密を入れない（環境変数でも公開は公開） | ✅ 接頭辞と秘密の語の組み合わせを見る。front-end はまだ無いが、増えた瞬間に見たいので先に置く |
| 167 | fast | `.env` を追跡しない（鍵の置き場所を許す以上、commit したら意味がない） | ✅ `.gitignore` の行と `git ls-files` の両方を見る。**ガードを書いた時点で `.gitignore` に `.env` が無かった** |
| 168 | fast | 待ち受けは 127.0.0.1 だけ（第2段の設定でノートを持ち出すと、隣の人からも見える） | ✅ `0.0.0.0` / `INADDR_ANY` / `[::]` を禁じ、`GuideServer` の `.loopback` と `mulmoterminal-agent-env` の既定が消えていないことを見る |
| 169 | fast | クラウド／BaaS の足場を黙って持ち込まない（足場が1つ増えた時点で第4段に入っている） | ✅ `vercel.json` / `wrangler.toml` / `firebase.json` / `supabase/` / `Dockerfile` … の追跡と、`package.json` の依存を見る。**宣言だけ D／第4段に上がって守りの検査が無い形も落とす**（「対策した」記憶だけが残るのは、記事 3 節の Supabase の IP 制限と同じ壊れ方） |
| 170 | governance | 段を飛ばさない（手元 → 社内ネットワーク → 入口と出口を既存ツールに任せる → クラウド）。上げる PR は宣言の変更と、その段の守りの検査をいっしょに持つ | |
| 171 | manual | 持ち出す Mac で、外向きに待っているものが無い（`lsof -iTCP -sTCP:LISTEN -P -n` に `*:` が出ない） | |
