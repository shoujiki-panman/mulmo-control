...
| 176 | fast | 自動の繋ぎ直しの知らせが、状態ごとに対応表どおり出る | ✅ `claudeRemoteNotes` を `StatusDisplay.swift` ごと組んで10通り**実際に走らせる**（止めた人には何も言わない・未信頼・ログイン時に起動しないときの注意を含む）。画面がその1本を通っていることも見る（#212） |
