...
| 164 | fast | 画面ガイドの本体がリポジトリに在り、接続先が Swift 側と揃っている | ✅ ファイルの存在と、`GuideServer.port` とスクリプトの接続先ポートの一致、MulmoTerminal のポートを決め打ちしていないことを見る。前の版は Tampermonkey の中にしか無く、拡張が消えた日に一緒に消えた（#202）。ポートが食い違っても何も落ちず、ガイドが出なくなるだけなので気づけない |
