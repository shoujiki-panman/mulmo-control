...
| 156 | fast | 行が増えても、一番下の `終了` が押せる | ✅ 収まらないぶんだけスクロールする入れ物を、タブの中身が通っていることと、`終了` がその外にあることを見る。#187 で行を1つ足したら画面からはみ出し、終了できなくなった |
| 157 | fast | 収まる高さを、作者の画面で決め打ちしない | ✅ 上限が `NSScreen` から決まっていることを見る。13インチのノートと外付けの大きい画面では収まる量が倍ちがう（#6 と同じ穴） |
