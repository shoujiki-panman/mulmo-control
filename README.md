...
session-relay（`relay` コマンド）を入れている場合は、同じ「スマホ連携」に `リレー` の行が出ます。**最初はオフ**で、行には「オフ」と `オン` だけが出ます（オフの間は何も走らせません）。`オン` を押すと、スマホから会話を預ける受け口と Cloudflare Tunnel を見張り、止まっていたら Mulmo Control が `relay doctor --fix` で起こして、行の下に「トンネルが止まっていたので 9/24 1:40 に起こしました」のように出します（アプリの起動時・パネルを開いたとき・5分おき）。`やめる` でオフに戻ります。
