...
「スマホ連携」の `Claude Code` の行で `繋ぐ` を押すと、既定のフォルダで `claude --remote-control` を1本立てます。**押したあとは、Mac の再起動や異常終了で消えても Mulmo Control が立て直します**（アプリの起動時と、5分おきの確認で）。立て直したときは行の下に「落ちたので 9/23 14:05 に繋ぎ直しました」のように出ます。

- `止める` を押したものは立て直しません
- 30分に3回落ちるようなら、立て直しをやめて「何度も落ちるので、自動の繋ぎ直しを止めました」と出します。`繋ぐ` で再開します
- 初めてのフォルダ（Claude Code の信頼確認がまだのもの）は立て直しません。確認は `繋ぐ` を押したときに開くターミナルで、本人が済ませてください
- **再起動のあと立て直すのは Mulmo Control 自身なので、上の `Mulmo Control を自動で起動` がオンのときだけです。** オフのときは行の下にそう出ます

session-relay（`relay` コマンド）を入れている場合は、同じ「スマホ連携」に `リレー` の行が出ます。スマホから会話を預ける受け口と Cloudflare Tunnel が止まっていたら、Mulmo Control が `relay doctor --fix` で起こし、行の下に「トンネルが止まっていたので 9/24 1:40 に起こしました」のように出します（アプリの起動時・パネルを開いたとき・5分おき）。

- 30分に3回起こすようなら、自動をやめて知らせます。`直す` で再開します
- 登録漏れなど、起こしても直らないものは起こしません。ターミナルで `relay doctor` を実行すると直し方が出ます
- `relay doctor --json` が使えない古い relay では「更新が要ります」と出ます
