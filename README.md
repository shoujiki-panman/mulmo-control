...
運用タブの `MulmoTerminal 画面ガイド` をオンにすると、MulmoTerminal の画面で
**カーソルを合わせたところに日本語の説明が出ます**。初めて触る人に「このボタンは
何か」を聞かれずに済ませるためのものです。

説明を描いているのはブラウザ側のユーザースクリプトで、この行はその ON/OFF を
切り替えます。**使うには一度だけ入れる作業が要ります。**

1. ブラウザに [Tampermonkey](https://www.tampermonkey.net/) を入れる
2. `userscript/mulmoterminal-guide.user.js` を開き、中身を新規スクリプトとして貼って保存する
3. MulmoTerminal の画面を開き直す

入れていないときは、この行をオンにしても何も起きません（アプリ側は待っているだけ
です）。逆にアプリを入れていなくても、画面右下の `?` から切り替えられます。

スクリプトはこのリポジトリが持っています。**拡張の中にしか置かないこと** — 前の
版はそうしていて、Tampermonkey が Mac から消えた日に一緒に消えました（#202）。
