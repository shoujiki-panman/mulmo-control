...
# ── 置き場所と公開範囲 ──────────────────────────────────────────
# SECURITY.md の O 節（164〜169）。中身は check-deploy.sh が持つ。分けてあるのは、
# あちらは grep しかしないので zsh だけで動き、Claude Code の hook が編集の
# たびに回せるから（ここは Swift のビルドが要る）。同じことを2箇所に書かない。
"${ROOT}/check-deploy.sh" || fail "置き場所と公開範囲の約束が破れています（check-deploy.sh）"
...
# 個人名を書かない。
#
# 公開リポジトリなので、書いた瞬間に誰でも読める。**利用者の報告を扱うときが
# 一番危ない** — 誰が何に困ったかは、本人の許可なく出してよい情報ではない。
#
# このガードが無かった間に3箇所すり抜けていた（START-HERE.md / KICKOFF.md /
# LEARNING-LOG.md）。全部のリリースを通り抜けている。**「守れていたつもり」
# ですらなく、最初から一度も見ていなかった。**
#
# 禁止したい名前を並べる形では勝てない（#83 の教訓）。名前は無限にあるし、
# 次に出てくる名前は今は分からない。**敬称のほうを見る。** 日本語で人を指すとき
# ほぼ必ず付くので、名前そのものを知らなくても捕まえられる。
#
# ただし敬称の字は普通の語にも混ざる（仕様・氏名・たくさん）。**許す語を並べて
# 先に消してから**、それでも敬称が残る行だけを見る。許していい語が出てきたら
# ここに1つ足す。
#
# check.sh 自身は対象外。このガードが探す語を、ここに書いてあるため。
NAME_OK='たくさん みなさん 皆さん お客さん 仕様 様子 同様 様々 多様 模様 氏名'
NAME_STRIP=()
for word in ${=NAME_OK}; do NAME_STRIP+=(-e "s/${word}//g"); done

HONORIFIC="$(grep -rn 'さん\|様\|氏' \
  --include='*.md' --include='*.sh' --include='*.swift' --include='*.json' --include='*.mjs' \
  "${ROOT}" 2>/dev/null \
  | grep -v '/\.git/' \
  | grep -v '/check\.sh:' \
  | sed "${NAME_STRIP[@]}" \
...
if [ -n "${HONORIFIC}" ]; then
  printf '%s\n' "${HONORIFIC}"
  fail "個人名が書かれています。公開リポジトリなので「利用者」などに置き換えてください"
fi
ok "個人名を書いていない"
